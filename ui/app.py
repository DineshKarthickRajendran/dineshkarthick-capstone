...
st.set_page_config(page_title="DK - CapStone Q&A", page_icon="🤖", layout="centered")
st.title("DK - CapStone Q&A")
st.caption("Ask anything — the API streams the answer back token by token.")

with st.form(key="ask_form", clear_on_submit=False):
    question = st.text_input(
        "Your question:", placeholder="e.g. What is the leave policy? "
    )
...
if submitted and question:
    ...
    try:
        with requests.post(
            API_URL, json={"question": question}, stream=True, timeout=60
        ) as response:
